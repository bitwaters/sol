...
> 后续 SEA 补验：已部署并验证重启、备份、旧镜像只读兼容、IPv4、一次性 Telegram 消息/告警。HTTP 断连和 429 在 SEA 无网络隔离容器的本机模拟服务中验证，未压测真实 GMGN 账户或切断生产网络。最终证据以 `RUNTIME-DEBUG-2026-09-14.md` 为准。
...
| 429 封禁演练 | 优先通过隔离模拟服务返回真实 HTTP 429，确认封禁期零请求与恢复；不以压测真实账户作为默认验收方式 |

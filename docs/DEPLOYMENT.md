...
新版本启动幂等创建 `position_jobs`、`cost_invalidation_job` 及持仓状态索引。部署脚本在重建后、切换容器前写入10分钟有效的计划重启标记，只作用于告警说明，不隐藏真实中断或清除缺口。运行时启用 `deferred_ingest`，采集成功以原始成交和任务均落盘为准。

验收检查三路心跳、入库各阶段耗时、后台任务数量/最早等待、处理失败次数和缺口状态。特别检查新数据不断进入时队列仍可排空。若回退到不认识后台队列的旧版本，必须先暂停新信号并停止采集，使用当前版本排空两个队列后才回退；不得直接忽略队列，否则持仓会落后于已入库成交。旧缺口按未恢复历史保留，不以关闭告警冒充补齐数据。

后台检查点验收：核对 `wal_checkpoint_failed=false`、`wal_checkpoint.at` 持续前进及剩余页数受控，同时确认三路入库最大耗时不再跟随检查点刷盘耗时。Worker 只执行 PASSIVE，不执行阻塞式 FULL/RESTART/TRUNCATE，不改变现有数据库同步级别。

### 共识状态单卡片验收

核验版本、`state_card_migration_v1` 与历史耗尽任务状态；对已推送信号检查 `state_message` 始终指向独立回复而非原信号。自然状态变动应增加编辑次数，不增加该信号的状态消息编号数量。已有重复消息保留。检查 `push_failure` 分类、`reply_block` 和管理员恢复通知；不能把 `unknown` 直接标记成功，也不能为修复卡片而重发首次信号。旧版回滚会恢复逐次新发的行为，应避免回滚到本次修复前的推送逻辑。

### GMGN Plus 限频验收（2026-09-23）

SEA 保持10权重/秒的项目预算，Plus账号总额度20/秒。关注钱包按每次10权重计费；容量10及发起平滑逻辑随GitHub代码部署，无需修改服务器凭证文件。检查启动日志中的configuredRate、effectiveRate、capacity、followWeight，及运行数据gmgn_limit_state。跨重启保留冷却和自适应降速值。对比部署后各端点limited计数与限频日志，不向GMGN发送额外压力测试流量。现有缺口仍保留完整性限制，不因429消失而标记已补齐。

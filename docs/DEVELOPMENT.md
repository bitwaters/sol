...
| `src/telegram` | 消息模板、推送、编辑节流、退出监控、命令 |
...
- 每个来源记录：最近成功时间、已完整处理的成交时间水位、已知缺口区间、回补 cursor（`source_health` 表）
...
- **水位与缺口判定（M1-15 冒烟修正）**：
  ...
  - 本页最旧事件已入库且回追到水位之前 → 连续，水位推进
  - 部分重叠但未追到水位 → 缺口 `[watermark, 回追点]`
  - 整页全新且无重叠 → 缺口 `[watermark, 本页最早时间]`（高流量溢出）
  - 缺口被后续重叠页覆盖后清除
  - **高流量源（smartmoney / kol）会持续整页返回，不得用"整页 = 缺口"，必须用重叠检测**
  - 缺口超过 10 分钟仍未修复 → 按"不可观测"接受（推进水位并记录），避免永久阻塞信号（门禁策略在 M2 实现）
...
- 触发后窗口继续滚动：新增计票 → 升级编辑（受 §7.5 节流约束）；钱包卖出 → 退出监控
...
缺口发现时即在同一采集事务内撤销受影响持仓的成本完整性，并持久化 `gap_affected:<token>:<wallet>` 及缺口末端时间。10 分钟超时只解除新信号的完整性门禁，不恢复已有持仓资格；仅重排当前数据库中的成交也不能证明缺失成交已补齐。缺口后的可靠零余额/成本完整检查点可作为重建恢复依据；仍有活跃缺口时不恢复。

### 7.5 冷却与升级

- 同 token 冷却 30 分钟（自**确认送达**时刻起）：冷却期内不产生新的信号消息，但已推送消息可以编辑升级
- **重新评估事件**：新增买入、卖出、窗口内买入过期、画像重分类 → 及时重算票数/保留率/价格比；评估与编辑分离，消息按节流更新
- 跌破门槛（票数不足 / 保留率 <60%）→ 标题改为"共识减弱"（不保留强共识）；全部簇退出按 §7.6 单独通知
- 升级记录：成员变更写入 `signal_wallets`（带 `joined_version`）与 `signal_evaluations`；`signals.snapshot` 为初始快照，写入后不可覆盖
- **消息修订号**：任何可见内容变化（成员增减、保留率 / 价格跨阈值、标题强弱变化）都递增 `signals.message_revision`；编辑任务绑定 `signal_id + escalate + revision`
- **最新修订检查**：发送编辑前检查任务 revision 是否等于当前 `message_revision`，旧任务直接丢弃，防止延迟重试覆盖新内容
- 成员版本（`escalated_count` / `joined_version`）与消息修订号**相互独立**，不混用
- 编辑节流：同一信号的编辑间隔 ≥30s；消息发出 30 分钟后停止编辑，后续变化改为发送跟进消息
- 全局推送上限 10 条/分钟（新信号与升级共享）；退出提醒优先级最高且不占用该额度
- 超限信号进入**持久化队列逐条延后发送**，发送前按 `signalTtlSeconds` 重验，过期则取消并记录；**第一版不做汇总消息**（关联/编辑规则复杂，见 §9.5）
...
- **去重**：按 `signal_id + 退出类型`（`consensus_exit` / `other_cluster_exit`）去重，不含升级版本；同一轮同类型只发一次，新一轮信号有新 signal_id 可正常提醒
- **监控终止**：token 年龄 >24h、所有绑定周期关闭、或确认送达后 24h，三者先到为准
...
```
🟢 强共识信号 #2N9C（已升级）
$PEPE2 · Pump.fun · 24 分钟
━━━━━━━━━━━━━━━━━━
市值 $210K · 流动性 $48K · 持有人 1,420

👥 15 分钟共识：6 票（+2）
🟡 仅 Smart Money ×3 · $10,600（建仓×2 / 加仓×1）
🟡🟠 Smart Money＋KOL ×1 · $3,000（建仓）
⭐ 仅自选 ×2 · $5,000（建仓）
💰 合计买入 $18,600 · 净流入 +$15,200
📊 持仓保留 85% · 现价 $0.00027 / 均价 $0.00022（+23%）

🛡 Rug 0.05 · Top10 17% · Bundler 5% · Insider 2% · 无 Wash · Dev 已清仓
🌐 TG · X
...
- 3 轮仍失败 → 运维频道告警（`TG_ALERT_CHAT_ID`），任务保留可人工补发
...
| **M3 推送** | grammY 推送、`push_tasks` 发送状态机、消息模板、编辑节流、升级复核、退出监控、命令与回调鉴权 | 测试群收到信号/升级/退出提醒；模拟发送超时→unknown→有限重试；编辑节流生效；按钮权限与 `/pause` 语义符合 §9.4 |

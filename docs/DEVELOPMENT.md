...
套餐总额度按权重计算：Free 5/5、Plus 20/20、Pro 50/50（每秒速率/容量）。2026-09-23 核对官方文档后，本项目默认预算为10权重/秒，使用容量10的本地桶及按权重平滑的发起时序；Plus 剩余额度不视为本项目可自动占用。

| 端点 | 权重 | 用途 |
|---|---|---|
| `GET /v1/user/smartmoney` | 1 | 聪明钱实时成交（买卖双边） |
| `GET /v1/user/kol` | 1 | KOL 实时成交（买卖双边） |
| `GET /v1/trade/follow_wallet` | 10 | 关注列表成交（signed，支持 next_page_token） |
| `GET /v1/user/wallet_activity` | 3 | 单钱包成交（备用） |
| `POST /v1/market/token_signal` | 1 | 服务端信号流（交叉验证用） |
...
> `GMGN_RATE_LIMIT_PER_SEC` 为本项目的权重预算，不是请求条数或个人 Key 的固定额度。关注钱包单次权重10，不能继续按旧版3计算；Free 套餐的容量小于该权重，不能仅通过降低轮询频率假定此接口可用。
> 官方依据：[track 权重及套餐](https://github.com/GMGNAI/gmgn-skills/blob/main/skills/gmgn-track/SKILL.md#rate-limit-handling)。
...
- 同一代币已发布信号在24小时内承接后续状态，普通状态提示独立回复首次消息；退出提醒首次单独回复，后续编辑同一条退出汇总。原30分钟编辑截止参数仅兼容旧配置，不控制新通知。
...
- 频道只接收首次信号及其增强、减弱、退出、失效、数据更正提示。后续消息引用首次消息，原信号永久不编辑；退出提醒复用首次退出消息，仅编辑该条汇总。
...
每个信号仅发送一条退出提醒，回复引用首次信号。后续清仓、迟到数据和重建纠正均更新这条退出汇总，展示当前共识退出、其他钱包退出、历史更正和持仓保留率。编辑间隔至少30秒，期间合并变化并保持单一待处理任务。重启从持久化消息定位信息恢复；旧版已发多条时复用最早一条已确认送达的退出消息。编辑失败走有限重试与管理员告警，不回退到新发消息；不删除历史频道消息、不编辑首次信号。

## 2026-09-21 倍率里程碑汇总

- 以首次信号实际发送时的有效价格及发送时间为固定基准；缺失或过期的发送价格不启用倍率统计，历史修复价格不冒充原始基准。既有信号仅接入剩余监测期，上线前的触达时间不回填。
- 从发送后24小时内的新鲜报价中观测，价格时间必须晚于发送时间、不得在未来，报价最多缓存60秒。沿用已有行情富化和限流，不额外逐钱包轮询。记录系统首次观测时间，可能晚于市场实际触达时间；暂停/屏蔽只影响发送，统计继续。
- 档位为1.5×、2×、3×、4×、5×及之后每个整数倍；跨档观测记录为闭区间，区间内每档共享该次首次观测时间，避免价格一次跳跃产生大量数据库行。`milestone_progress:<id>` 持久化原始基准、累计最高达标档位及所有区间的报价/报价时间/观测时间。每档用时为观测时间减原信号发送时间，回落与再创新高以下档位不重复触发。
- 第一次达标回复引用原信号。新档位到达后发送最新汇总，确认成功并持久化新消息定位后删除上一条倍率汇总。原信号和退出提醒均不编辑、不删除。每条信号至少间隔60秒新发一次，并共享全局每分钟发送预算；期间变化合并，消息最多展示最近4档。
- `milestone_message:<id>` 保存最后成功消息；`milestone_cleanup:<id>` 单独保存待删除旧消息及有限重试状态。删除失败不重发，未清理完之前停止该信号新的倍率汇总；三次失败后私发管理员告警。发送结果不明或发送期间崩溃停止自动补发，由管理员核对。所有已观测档位继续统计。
- `/stats` 增加上线后累计的1.5×、2×、3×、5×达标信号数；完整档位记录保存在数据库，不混入研究样本准入规则。投递重放保存倍率基准、观测、消息定位和清理状态，并统计模拟删除次数。

## 2026-09-21 采集与持仓计算解耦

运行时采集事务仅写入原始成交、来源观测、健康水位、缺口记录和 `position_jobs`；按钱包/代币合并最早受影响成交时间，后台逐项处理持仓及乱序重放。持仓更新与任务删除同事务，失败回滚并退避，重启继续处理。存在待处理持仓的代币暂停资格评估、首次发送和状态/退出提示；异步富化返回后再次检查，避免以新成交配旧持仓发送信号。独立的价格倍率统计继续运行。完整性研究快照也保存队列并标为待处理，而不是合格样本。
...
## 2026-09-22 共识状态单卡片与失败生命周期

- 每条首次信号保持不可变。共识增强/减弱/恢复、失效、数据不可核验及追高警告共用一张回复原信号的状态汇总卡。首次发送后保存 `state_message:<id>`，后续只编辑这张卡；至少间隔30秒（配置更大时使用配置值），期间合并为最新有效修订。
- 升级时兼容已有消息：复用最早一条确认成功、且消息编号不同于原信号的状态回复。此前已产生的重复消息不批量删除。退出仍使用独立的单张退出汇总；倍率仍按已确认需求新发最新汇总、成功后删除旧汇总，失败不继续新发。
- 编辑前持久化目标消息编号。首次状态卡片发送结果不明（含进程崩溃）时，保留 `unknown` 及 `reply_block:<signalId>:escalate`，后续新修订也不得再新发。已有卡片的编辑属于可重试操作；失败不回退为新消息。原信号不存在或汇总不可编辑时，取消无效任务，记录阻断原因，并私发管理员。
- `push_failure:<taskId>` 保存脱敏失败分类和历史处理依据。确定失败的跟进任务过期或被新修订替代后取消；送达不明记录不自动归档。重试耗尽告警列出任务编号、信号编号和失败原因；解除文案说明队列状态变化，不宣称历史消息已补发。
- 首次升级执行一次迁移：仍有效且已有独立确认回复的旧版耗尽状态任务，可重置重试预算，改为编辑已确认回复；无锚点或被阻断的任务不恢复新发。迁移标记 `state_card_migration_v1` 防止重启后重复重置预算。
- 投递重放保存新锚点、迁移标记和各回复类型的阻断状态。正式频道不发送合成测试消息；通过自然状态变化验证编辑链路。

## 2026-09-23 GMGN 限频修复

关注钱包单页请求权重从过时的3纠正为10，持仓端点为2。生产及只读调试入口统一使用能容纳最大端点权重的容量10；所有前台、回补和研究请求继续共享预算，后台保留让路与单并发限制。请求发起串行分配时间，每次实际发起后按权重/有效速率确定下一时刻，事件循环卡顿或冷却期间不积攒可同时释放的发送许可；网络响应仍允许并发。

`gmgn_limit_state` 持久化冷却截止时间、错误类别、最近限频时间及有效预算；启动恢复尚未结束的冷却。每次新的限频事件有效预算乘0.8，最低为min(配置预算,2)，同一冷却期间的在途失败只延长封禁、不重复降速。降速不会在重启后被清除，也不会自动探测上游极限；只有核对套餐和共享调用情况后才应人工恢复预算。配置预算下调时以较小值为准。未知冷却时间保守等待5分钟，已过期的恢复时间至少等待1秒，优先遵守reset头、正文reset_at和Retry-After中较晚的时间。

限频检测绑定实际传输 Promise，即使调用方20秒兜底已超时，迟到的429也会关闭共享门。冷却只阻止新请求，已经发出的在途请求无法撤回；其返回更晚的封禁时间仍被保存。独立进程/其他项目没有共享本数据库和调度器，不属于单实例网关的限流保障范围。


### 2026-09-23 积压与分页容错修复

后台等待前台队列清空会在持续富化时饥饿，现改为每秒最多1权重、单并发的公平排队；所有请求仍共享原全局速率、按实际端点权重间隔发起和持久化封禁。后台10秒排队截止，取消不破坏后续请求顺序。部分分页已成功后下一页失败，先事务保存已有成交与真实成功时间，再按错误退避；不把失败页视为完整数据。

研究到期未完成项达到100且最早逾期超过30分钟时，持续向管理员提示积压；判断基于原到期时间，不因重试延期或少量检查成功而掩盖。这里的时间只用于运维告警，不是调参门槛。增加事件循环最大阻塞与研究采样/富化/实验检查耗时，区分本地阻塞和网络等待。历史缺行情及缺口记录不改写为成功。

同次验证还发现研究实验检查阻塞主线程约7.4秒。首次样本改为先在SQL按版本、代币、入选时间和ID选定记录，再读取诊断正文；失败的首次记录仍保留，不能用后续成功记录替换。同一报告的多个阈值方案复用本次缺口核对结果，下次报告重新核对，确保后发现的缺口仍撤销资格。SEA同一只读快照的2116个独立样本完整报告逐字段一致，实测耗时从约6.90秒降至1.43秒；仍为同步检查，后续应持续关注数据增长后的耗时。

候选评估的网络等待并发由2提高到4，按代币去重及运行中更新后的重验不变，仍受同一GMGN全局预算和公平队列限制。运行快照新增候选等待数量、执行数量及最早等待时间，避免仅看已完成任务的延迟分位数推断当前积压。

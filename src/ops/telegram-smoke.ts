...
if (mode === '--preview') {
  console.log(JSON.stringify({ productionPusherStarted: false, sends: ['signal', 'signal_upgrade', 'exit', 'ops_alert'], edits: [], messages: { signal, upgrade, exit, alert } }, null, 2));
} else {
  if (!['--preflight', '--send-test'].includes(mode)) throw new Error('Unknown mode');
  try {
    const { env } = loadConfig();
    if (!env.TG_BOT_TOKEN || !env.TG_CHAT_ID) throw new Error('Telegram test configuration missing');
    const admin=adminRecipients((env.TG_ADMIN_IDS??'').split(','),env.TG_CHAT_ID)[0];
    if(!admin)throw new Error('No private admin target');
    const bot = new Bot(env.TG_BOT_TOKEN);
    const me = await bot.api.getMe();
    const chat = await bot.api.getChat(env.TG_CHAT_ID);
    const alertChat = await bot.api.getChat(admin);
    const member = await bot.api.getChatMember(env.TG_CHAT_ID, me.id);
    const alertMember = await bot.api.getChatMember(admin, me.id);
    if (mode === '--preflight') {
      ...
      await bot.api.sendMessage(env.TG_CHAT_ID, upgrade, {parse_mode:'HTML',reply_parameters:{message_id:sent.message_id}});
      ...
      started = performance.now();
      await bot.api.sendMessage(admin, alert);
      timings.alertMs = Math.round(performance.now() - started);
      console.log(JSON.stringify({ messagesSent: 4, messagesEdited: 0, productionPusherStarted: false, timings }));
    }
  } catch (error) {
